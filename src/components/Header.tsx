    ...
    <>
        ...
        <div className="container-premium flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-8">
            {siteConfig.nav.map((item) => (
              <button key={item.target} onClick={() => handleNav(item.target)} className="link-underline text-sm font-medium text-ivory-200 transition-colors duration-300">{item.label}</button>
            ))}
            <button onClick={handleEnquire} className="btn-primary !py-3 !px-6">Enquire Now</button>
          </nav>
      ...
      <div className={`fixed inset-0 z-[60] lg:hidden transition-all duration-500 ease-premium ${mobileOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className="relative h-full flex flex-col justify-center px-8">
          ...
          <button onClick={handleEnquire} className="btn-primary mt-12 w-full sm:w-auto sm:self-start" style={{opacity: mobileOpen ? 1 : 0, transform: mobileOpen ? 'translateY(0)' : 'translateY(20px)', transition: 'opacity 0.5s ease 0.6s, transform 0.5s ease 0.6s'}}>Enquire Now</button>
          <div className="absolute bottom-8 left-8 right-8 flex justify-between items-center pt-6 border-t border-ivory-200/10" style={{opacity: mobileOpen ? 1 : 0, transition: 'opacity 0.5s ease 0.8s'}}><span className="text-xs uppercase tracking-[0.3em] text-ivory-300">Nexus Travel Group</span><span className="text-xs text-ochre-400">See more. Go further.</span></div>
        </div>
      </div>
    </>
  );
